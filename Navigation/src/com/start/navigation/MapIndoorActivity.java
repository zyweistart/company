...
import com.start.service.FloorIndexAdapter;
...
public class MapIndoorActivity extends MapActivity implements OnItemClickListener, OnTapMapListener.OnClickListener, View.OnClickListener, PathSearchListener {
	...
	protected Room[][] mRooms;

	protected FloorIndexAdapter mFloorAdapter;
	protected ListView mListView;
	protected int mListViewCheckedPos;

	protected MapView mMapView;
	protected ListOverlay mListOverlay;
	...
	protected Dialog onCreateDialog(int id) {
		...
		case Utils.DLG_SEARCH_OPTION:
			...
			break;
		case Utils.DLG_POI:
			dialog = createPOIDialog();
			break;
		case Utils.DLG_EXIT_NAVIGATION:
			dialog = Utils.buildDialog(this, R.string.exit_navigation, new DialogInterface.OnClickListener() {
				@Override
	...
	private Dialog createPOIDialog() {
		POIDialog poiDialog = new POIDialog(this, R.style.dialog_poi);
	...
	private Dialog createSearchOptionDialog() {
	...
	private void updateOverlay() {
		...
		ArrayList<OverlayItem> markers = getMarkers(currentFloor);
		Polyline routeLine = getRouteLine(currentFloor);
	...
	protected ArrayList<OverlayItem> getMarkers(int currentFloor) {
	...
	protected Polyline getRouteLine(int currentFloor) {
		if (AppContext.getInstance().getPathSearchResult() == null) {
			return null;
		}
	...
	private android.view.View.OnTouchListener onTouchListener = new OnTouchListener() {
...
//					intent.setClass(this, OfficeActivity.class);
	...
	protected Paint getPaintStroke() {
	 ...
	 * 添加当前自己位置点的标记
	 */
	protected void addMyLocMarker(MyLocation myLocation) {
	...
	protected void removeMyLocMarker() {
		if (mMyLocMarker != null) {
			List<OverlayItem> itemList = mListOverlay.getOverlayItems();
			synchronized (itemList) {
				if (itemList.contains(mMyLocMarker)) {
