//
//  ACExtractionListViewController.m
//  ACyulu
//
...
@interface ACExtractionListViewController ()

@end

@implementation ACExtractionListViewController{
...
- (id)initWithNibName:(NSString*)nibNameOrNil bundle:(NSBundle*)nibBundleOrNil{
    ...
    if(self){
       self.navigationItem.title=@"提取列表";
    }
    return self;
}
...
- (void)viewDidLoad{
    ...
    if(dictioanry){
        id content=[dictioanry objectForKey:CACHE_EXTRACTIONLIST];
        if(content){
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if(!self.dataItemArray||[self.dataItemArray count]==0||[[Config Instance]isRefreshExtractionList]){
        [self autoRefresh];
        [[Config Instance]setIsRefreshExtractionList:NO];
    }
    [[BaiduMobStat defaultStat] pageviewStartWithName:@"ACExtractionListViewController"];
}

- (void)viewDidDisappear:(BOOL)animated{
    [[BaiduMobStat defaultStat] pageviewEndWithName:@"ACExtractionListViewController"];
}
...
- (void)requestFinishedByResponse:(Response*)response requestCode:(int)reqCode{
    ...
    if([response successFlag]){
        if([[response code]isEqualToString:@"110042"]||_currentPage==1){
            ...
            [dictionary setObject:[response responseString] forKey:CACHE_EXTRACTIONLIST];
            //缓存数据
...
- (void)reloadTableViewDataSource{
	if([[Config Instance]isLogin]){
        ...
        [requestParams setObject:@"1" forKey:@"accstatus"];
